...
// 储存离线消息
void OfflinMsgModel::insert(int id, const std::string& msg)
{
    ...
    const char *query = "INSERT INTO offlinemessage(userid, message) VALUES(?, ?)";
    ...
    bind[1].buffer_length = msgLen;
    bind[1].length = &msgLen;
...
// 查询用户的离线消息
std::vector<std::string> OfflinMsgModel::query(int id)
{
    MySQL mysql;
    std::vector<std::string> vec;
    if (!mysql.connect()) return vec;

    MYSQL_RES *res = mysql.query("SELECT message FROM offlinemessage WHERE userid = " + std::to_string(id));
    if (res != nullptr)
    {
        MYSQL_ROW row;
